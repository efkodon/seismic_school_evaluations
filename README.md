...
This project revealed inconsistent building information for some of the school buildings found in both state and local data sources, which may affect the earthquake risk levels assigned to individual school buildings and campuses in the ICOS dataset. As a result, the full ICOS dataset is not published in this repository. However, excerpts from the dataset are shown within the coding documents. Data documentation, including a glossary, provided with the dataset by the state's Office of Superintendent of Public Instruction is available here.
